...
class View {
	...
	public function render($echo = true)
	{
		...
		$content = preg_replace('#<script( .+)? src="'.BASE_URL.'http(s)?://(.+)"( .+)?>#isU', '<script$1 src="http$2://$3"$4>', $content);
		$content = preg_replace('#<a( .+)? href="(.+)"( .+)?>#isU', '<a$1 href="'.BASE_URL.'$2"$3>', $content);
		$content = preg_replace('#<a( .+)? href="'.BASE_URL.'http(s)?://(.+)"( .+)?>#isU', '<a$1 href="http$2://$3"$4>', $content);
